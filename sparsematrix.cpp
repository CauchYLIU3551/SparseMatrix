...
private:
	std::vector<int> rowoffset; // A vector that saves the data of the rowoffsets info;
	std::vector<int> indice; // A vector that saves the col index of the nonzero element;
	std::vector<double> value; // a vector that saves the nonzero values; 
	int col;
	int row;
	int max();
};
...
sparsematrix sparsematrix::diag()
{
	...
	for (int i = 0; i < row; i++)
	{
		...
		if (flag != 1)
		{
			printf_s("In the %dth row the element in the diag is zero\n", i + 1);
			r.push_back(r[i]);
		}
